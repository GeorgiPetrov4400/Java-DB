...
#8 Games of 2022
SELECT  g.`name`, 
		g.`release_date`, CONCAT(SUBSTRING(g.`description`, 1, 10), '...') AS 'summary', 
		(CASE
...
WHERE g.`name` LIKE '%2' AND YEAR(g.`release_date`) = 2022 AND MONTH(g.`release_date`) % 2 = 0
ORDER BY `quarter`;
...
DETERMINISTIC
BEGIN
	RETURN (SELECT CONCAT_WS(' ','The', g.`name`, 'is developed by a', t.`name`, 'in an office with an address', a.`name`) FROM `games` AS g
	JOIN `teams` AS t ON g.`team_id` = t.`id`
	JOIN `offices` AS o ON t.`office_id` = o.`id`
	JOIN `addresses` AS a ON o.`address_id` = a.`id`
