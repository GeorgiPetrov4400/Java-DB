...
#8 Top from menu
SELECT p.`id`, p.`name`, COUNT(`product_id`) AS 'count' FROM `products` AS p
...
WHEN (t.`capacity` - COUNT(c.`id`) = 0) THEN 'Full'
ELSE 'Extra seats'
END) AS 'availability' 
FROM `tables` AS t
...
DETERMINISTIC
BEGIN
	RETURN (SELECT SUM(p.`price`) AS 'bill' FROM `clients` AS c
...
#11 Happy hour
CREATE PROCEDURE udp_happy_hour (enter_type VARCHAR(50))
BEGIN 
	UPDATE `products`
    SET `price` = `price` * 0.8
	WHERE `type` = enter_type AND `price` >= 10;
END %%