...
INSERT INTO `reviews` (`content`, `rating`, `picture_url`, `published_at`) 
		(SELECT SUBSTRING(p.`description`, 1, 15), (p.`price` / 8), REVERSE(p.`name`), DATE '2010-10-10' FROM `products` AS p
		WHERE p.`id` >= 5);
...
BEGIN
    ...
    SET products_count := (SELECT COUNT(p.`id`) AS 'total_products' FROM `customers` AS c
						   JOIN `orders` AS o ON c.`id` = o.`customer_id`
